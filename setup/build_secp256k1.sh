...
(cd src && ./configure --host=$HOST_ARCH-apple-darwin \
	CC=`xcrun -find clang` \
	CFLAGS="-O3 -arch $HOST_ARCH -isysroot \
	`xcrun -sdk iphonesimulator --show-sdk-path` -fembed-bitcode-marker -mios-simulator-version-min=8.0" \
	CXX=`xcrun -find clang++` \
	CXXFLAGS="-O3 -isysroot \
	`xcrun -sdk iphonesimulator --show-sdk-path` -fembed-bitcode-marker -mios-simulator-version-min=8.0" --prefix="$TARGETDIR_IPHONEOS" \
	&& make install)
...
(cd src && ./configure --host=$HOST_ARCH-apple-darwin \
 	...
 	CXX=`xcrun -find clang++` \
 	CXXFLAGS="-O3 -isysroot \
 	`xcrun -sdk iphoneos        --show-sdk-path` -fembed-bitcode -mios-version-min=8.0" --prefix="$TARGETDIR_SIMULATOR" \
 	&& make install)
